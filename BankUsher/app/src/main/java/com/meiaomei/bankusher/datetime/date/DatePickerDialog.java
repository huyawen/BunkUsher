...
package com.meiaomei.bankusher.datetime.date;
...
import com.meiaomei.bankusher.datetime.HapticFeedbackController;
import com.meiaomei.bankusher.datetime.TypefaceHelper;
import com.meiaomei.bankusher.datetime.DateTimeUtils;
...
public class DatePickerDialog extends DialogFragment implements
    ...
    private Calendar mCalendar = DateTimeUtils.trimToMidnight(Calendar.getInstance(getTimeZone()));
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (!mThemeDarkChanged) {
            mThemeDark = DateTimeUtils.isDarkTheme(activity, mThemeDark);
        }
        ...
        if (mAccentColor == -1) {
            mAccentColor = DateTimeUtils.getAccentColorFromThemeIfAvailable(getActivity());
        }
        if (mDatePickerHeaderView != null) mDatePickerHeaderView.setBackgroundColor(DateTimeUtils.darkenColor(mAccentColor));
        ...
        switch (viewIndex) {
            case MONTH_AND_DAY_VIEW:
                if (mVersion == Version.VERSION_1) {
                    ObjectAnimator pulseAnimator = DateTimeUtils.getPulseAnimator(mMonthAndDayView, 0.9f,
                            1.05f);
                    if (mDelayAnimation) {
                ...
                String dayString = android.text.format.DateUtils.formatDateTime(getActivity(), millis, flags);
                mAnimator.setContentDescription(mDayPickerDescription + ": " + dayString);
                DateTimeUtils.tryAccessibilityAnnounce(mAnimator, mSelectDay);
                break;
            case YEAR_VIEW:
                if (mVersion == Version.VERSION_1) {
                    ObjectAnimator pulseAnimator = DateTimeUtils.getPulseAnimator(mYearView, 0.85f, 1.1f);
                    if (mDelayAnimation) {
                        pulseAnimator.setStartDelay(ANIMATION_DELAY);
                        mDelayAnimation = false;
                    }
    ...
    private void updateDisplay(boolean announce) {
        ...
        String monthAndDayText = android.text.format.DateUtils.formatDateTime(getActivity(), millis, flags);
        mMonthAndDayView.setContentDescription(monthAndDayText);

        if (announce) {
            flags = android.text.format.DateUtils.FORMAT_SHOW_DATE | android.text.format.DateUtils.FORMAT_SHOW_YEAR;
            String fullDateText = android.text.format.DateUtils.formatDateTime(getActivity(), millis, flags);
    ...
    public void setHighlightedDays(Calendar[] highlightedDays) {
        for (Calendar highlightedDay : highlightedDays) DateTimeUtils.trimToMidnight(highlightedDay);
    ...
    public boolean isHighlighted(int year, int month, int day) {
        ...
        DateTimeUtils.trimToMidnight(date);
