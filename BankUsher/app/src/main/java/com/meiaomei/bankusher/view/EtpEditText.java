...
public class EtpEditText extends EditText {
    ...
    public EtpEditText(Context context) {
        this(context, null);
    }

    public EtpEditText(Context context, AttributeSet attrs) {
        //指定了默认的style属性
        this(context, attrs, android.R.attr.editTextStyle);
    }

    public EtpEditText(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        if (visibilityDrawable == null) {
            visibilityDrawable = getResources().getDrawable(R.mipmap.yingcang);
        }
    }
        ...
        if (action == MotionEvent.ACTION_UP) {
            if (getCompoundDrawables()[2] != null) {
                ...
                if (xFlag) {
                    ...
                    if (visibililty) {
                        visibilityDrawable = getResources().getDrawable(R.mipmap.signin_eye_on);
                        /*this.setInputType(EditorInfo.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD);*/
                        this.setTransformationMethod(HideReturnsTransformationMethod.getInstance());
                    } else {
                        //隐藏密码
                        visibilityDrawable = getResources().getDrawable(R.mipmap.signin_eye_off);
