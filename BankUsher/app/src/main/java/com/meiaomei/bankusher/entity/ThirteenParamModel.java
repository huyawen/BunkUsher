package com.meiaomei.bankusher.entity;

/**
 * Created by huyawen
 * 13个参数的公用类
 * 1 visitTime
 * 2 visitAddress
 * 3 faceId
 * 4 name
 * 5 phoneNumber
 * 6 sex
 * 7 idnumber
 * 8 viporder
 * 9 remark
 * 10 imgurl
 * 11 visitId
 * 12 WorkNumber
 * 13
 */
public class ThirteenParamModel {//13个参数的类
    long firstPara;
    String secondPara;
    String thirdPara;
    String fourthPara;
    String fifthPara;
    String sixthPara;
    String seventhPara;
    String eighthPara;
    String ninthPara;
    String tenthPara;
    String eleventhPara;
    String twelfthPara;
    String thirteenthPara;

    public String getEleventhPara() {
        return eleventhPara;
    }

    public void setEleventhPara(String eleventhPara) {
        this.eleventhPara = eleventhPara;
    }

    public String getTwelfthPara() {
        return twelfthPara;
    }

    public void setTwelfthPara(String twelfthPara) {
        this.twelfthPara = twelfthPara;
    }

    public String getThirteenthPara() {
        return thirteenthPara;
    }

    public void setThirteenthPara(String thirteenthPara) {
        this.thirteenthPara = thirteenthPara;
    }

    public long getFirstPara() {
        return firstPara;
    }

    public void setFirstPara(long firstPara) {
        this.firstPara = firstPara;
    }

    public String getSecondPara() {
        return secondPara;
    }

    public void setSecondPara(String secondPara) {
        this.secondPara = secondPara;
    }

    public String getThirdPara() {
        return thirdPara;
    }

    public void setThirdPara(String thirdPara) {
        this.thirdPara = thirdPara;
    }

    public String getFourthPara() {
        return fourthPara;
    }

    public void setFourthPara(String fourthPara) {
        this.fourthPara = fourthPara;
    }

    public String getFifthPara() {
        return fifthPara;
    }

    public void setFifthPara(String fifthPara) {
        this.fifthPara = fifthPara;
    }

    public String getSixthPara() {
        return sixthPara;
    }

    public void setSixthPara(String sixthPara) {
        this.sixthPara = sixthPara;
    }

    public String getSeventhPara() {
        return seventhPara;
    }

    public void setSeventhPara(String seventhPara) {
        this.seventhPara = seventhPara;
    }

    public String getEighthPara() {
        return eighthPara;
    }

    public void setEighthPara(String eighthPara) {
        this.eighthPara = eighthPara;
    }

    public String getNinthPara() {
        return ninthPara;
    }

    public void setNinthPara(String ninthPara) {
        this.ninthPara = ninthPara;
    }

    public String getTenthPara() {
        return tenthPara;
    }

    public void setTenthPara(String tenthPara) {
        this.tenthPara = tenthPara;
    }
}
